...
.PHONY: all
all: lint ## Run all tests and lint (default)
...
.PHONY: lint
lint: lint-proto  ## Lint code and protos
...
generate-proto: $(BIN)/buf
	rm -rf src/main/java/build/buf/validate
	rm -rf src/main/java/build/tests
	$(BIN)/buf generate
...
$(BIN)/protovalidate-conformance: $(BIN) Makefile
	GOPRIVATE=github.com/bufbuild/protovalidate-api/ \
	GONOPROXY=github.com/bufbuild/protovalidate-api/ \
	GONOSUMDB=github.com/bufbuild/protovalidate-api/ \
	GOBIN=$(abspath $(BIN)) $(GO) install \
    	github.com/bufbuild/protovalidate-api/tools/protovalidate-conformance@latest
